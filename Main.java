...
public class Main {
	public static void main(String[] args) {
		try (Connection conexao = ConexaoDB.conectar()) {
			...
			//Lista todos produtos apos a inserção
			mostrarProdutos(produtoDAO);

			//Exemplo de consulta por ID (consulta o produto com ID 1)
	...
	// Metodo para listar os produtos
		private static void mostrarProdutos(ProdutoDAO produtoDAO) {
			List<Produto> todosProdutos = produtoDAO.listarTodos();
				if (todosProdutos.isEmpty()) { //verifica se a lista está vazia.
					System.out.println("Nenhum produto encontrado");
				} else {
					System.out.println("Lista de Produtos");
						for (Produto p : todosProdutos) {
							System.out.println(p.getId() + ": " + p.getNome() + "_ " + p.getPreco());
						}
				}
		}
}